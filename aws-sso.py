...
if need_login:
    ...
    auth_factor = input(
        'Enter an authentication factor ("push", "phone", "sms") or Duo passcode. Or press enter to use your default factor: '
    )
    ...
    if auth_factor == "":
        ...
        payload["factor"] = "sms"
        session.post(duourlprompt, data=payload, verify=sslverification)
    ...
    response = session.post(duourlprompt, data=payload, verify=sslverification)
    ...
    response = session.post(duourlstatus, data=payload, verify=sslverification)

    duourlstatus = response.url

    response = session.post(
        duourlstatus + "/" + payload["txid"], data=payload, verify=sslverification
    )
    ...
    response = session.post(duourl, data=payload, verify=sslverification)
    ...
    if assertion == "":
        print(
            "Response did not contain a valid SAML assertion. Are you being prompted to change your password at login?"
        )
        sys.exit(0)
...
if len(awsroles) > 1:
    ...
    try:
        ...
        print(
            "Could not find role ARN. Are you sure you logged in with an ID that has AWS access?"
        )
        sys.exit(0)
...
print(
    f'Your new access key pair has been stored in {filename}, in the "saml" profile section.'
)
print(
    f"This key pair will expire in {token_duration} seconds, at {expiration_formatted} (in your local timezone)."
)
print(
